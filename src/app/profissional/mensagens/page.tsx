...
export default async function MensagensProfissionalPage() {
  const user = await requireUser("PROFISSIONAL");
  const conversations = await getConversationsForUser(user.id, "PROFISSIONAL");
      ...
      <main className="mx-auto max-w-2xl px-4 py-6">
        ...
        <div className="mt-5 space-y-2">
          ...
          {conversations.map((c) => (
            <Link
              key={c.id}
              href={`/profissional/mensagens/${c.id}`}
